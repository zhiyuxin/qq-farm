...
class MiniProgramLoginSession {
    ...
    static async getAuthCodeResult(ticket, appid = '1112386029') {
        try {
            ...
            const rawCode = payload.data && payload.data.code !== undefined
                ? payload.data.code
                : payload.code;
            const code = String(rawCode || '').trim();
            const message = payload.message || payload.msg || payload.errmsg
                || (payload.data && (payload.data.message || payload.data.msg || payload.data.errmsg))
                || '';

            if (!code || /^-?\d+$/.test(code)) {
                const errCode = payload.code !== undefined ? payload.code : (code || 'empty');
                const error = message ? `code=${errCode}, message=${message}` : `code=${errCode}`;
                console.warn(`[MP getAuthCode] API error: ${error}`);
                return { code: '', error };
            }
